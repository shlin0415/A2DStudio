...
格式：先输出说话者标记，然后换行输出"旁白："前缀 + 描述内容
{"speaker":"narrator"}
旁白：要描述的文本
- 旁白行不绑定任何角色，前端居中灰色字幕显示
- 可以有语音（按旁白音源合成），也可以纯字幕
- 旁白是可选能力，只在需要叙事时使用，不需要每行都带旁白
- 不要在"旁白："前缀后使用<TTS>标签，旁白文本直接作为朗读内容

示例1：
{"speaker":"narrator"}
旁白：夕阳把教室染成橙红色，放课后的铃声响起。
