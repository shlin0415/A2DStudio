...
export const useUIStore = defineStore("ui", {
  ...
  actions: {
    ...
    showNotification(options: {
      ...
      const debounceMs = isDefaultError
        ? DEBOUNCE_MS_NETWORK
        : DEBOUNCE_MS_DEFAULT;
      ...
      if (now - lastTime < debounceMs) {
        console.log(
          `跳过重复通知：${title}（${debounceMs / 1000}秒内已显示过）`,
        );
        return;
      }
      ...
      this.notification = {
        ...
        message,
        avatarUrl:
          avatarUrl || `/characters/${this.currentCharacterFolder}/头像.png`,
        duration,
      };
    ...
    getRefreshTip(type: "success" | "fail") {
      ...
      return (
        this.tipsMap[key] || {
          ...
          message:
            type === "success" ? "角色列表已成功刷新！" : "刷新时出了问题",
        }
      );
    },
