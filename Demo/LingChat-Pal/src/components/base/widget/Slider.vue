<template>
  <div>
    <span>
      <slot name="left">{{ leftLabel }}</slot>
    </span>
    <input type="range" :min="min" :max="max" :step="step" :value="value" @input="$emit('input', Number(value))"
      @change="$emit('change', Number(value))" v-model="value" />
    <span>
      <slot name="right">{{ rightLabel }}</slot>
    </span>
  </div>
</template>
