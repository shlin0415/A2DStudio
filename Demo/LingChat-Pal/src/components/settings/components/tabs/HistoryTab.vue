<template>
  <article class="w-full h-full flex flex-col min-h-0">
    ...
    >
      <div>
        <h2
          class="text-xl font-black tracking-wide mb-1 transition-colors flex items-center gap-2"
    ...
    <div class="flex flex-col flex-1 min-h-0 gap-3">
      ...
      <div v-else class="flex flex-col flex-1 min-h-0 gap-4">
        <!-- 滚动对话区域 -->
        <div
          class="flex-1 min-h-0 overflow-y-auto p-4 rounded-xl border shadow-sm transition-all"
          :class="
            isDarkMode
